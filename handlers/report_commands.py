...
class ReportCommandHandlers:
    ...
    async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        stories = ReportCommandHandlers.story_db.get_user_stories(user.id)

        cutoff = (datetime.utcnow() - timedelta(weeks=2)).date()
        stories = [s for s in stories if s['created_at'][:10] >= str(cutoff)]

        if not stories:
            await update.message.reply_text(
                "No stories found in the last 2 weeks. Use /story to record a moment!"
            )
            return
        ...
        if len(full_message) <= TELEGRAM_MAX_LENGTH:
            ...
            # Split into chunks that fit Telegram's limit, breaking on newlines where possible
            chunks = _split_text(report_text, TELEGRAM_MAX_LENGTH - len(header))
            await update.message.reply_text(header + chunks[0], parse_mode='HTML')
            for chunk in chunks[1:]:
                await update.message.reply_text(chunk, parse_mode='HTML')


def _md_to_html(text: str) -> str:
    # Escape HTML entities first so our tags aren't double-escaped
    text = html.escape(text)

    # Horizontal rules → thin separator
    text = re.sub(r'^-{3,}$', '─────────────', text, flags=re.MULTILINE)

    # Headers (##, ###, ####) → bold + blank line
    ...
    # Bullet lists before italic so "* item" isn't treated as emphasis
